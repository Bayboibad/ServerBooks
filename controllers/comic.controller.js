...
exports.listComic = async (req, res, next) => {
  ...
  var list = await Comic.find(dieu_kien_loc).populate(
    "author",
    "fullName birthday nationality image biography"
  );
  res.send(list);
};
...
exports.addComic = async (req, res, next) => {
  upload.fields([
    ...
    const contentImages = req.files["contentImage"].map(
...
exports.addComicInApp = async (req, res, next) => {
  upload.fields([
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: "Thêm truyen tranh thành công", newItem: newComic });
    } catch (error) {
      console.error(error);
...
exports.updateComic = async (req, res, next) => {
  upload.fields([
    ...
    if (!req.files["contentImage"] || !req.files["contentImage"].length) {
      ...
      contentImages = req.files["contentImage"].map(
        (file) => "uploads/" + file.filename
      ); // Mảng ảnh nội dung
    }
...
exports.updateComicInApp = async (req, res, next) => {
  upload.fields([
    ...
    try {
      const newComic = await Comic.updateOne(
        { _id: _id },
        {
          name: name,
          author: author,
      ...
      res.status(201).json({
        message: "Cap nhat truyen tranh thành công",
        newItem: newComic,
      });
    } catch (error) {
      console.error(error);
